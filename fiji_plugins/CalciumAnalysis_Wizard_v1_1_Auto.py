# CalciumAnalysis_Wizard_v1_1_Auto.py
# ImageJ/Fiji Plugin for Calcium Transient Analysis - V1.1 AUTO
# Cytoplasmic ring ROIs: measures calcium in ring around nucleus (excludes nuclear signal)
#
# VERSION: v1.1-auto (cytoplasmic ring ROIs via ShapeRoi XOR)
#
# Key difference from v1-auto:
#   - Creates "donut" ROIs: cytoplasm ring = dilated_nucleus XOR nucleus
#   - Excludes nuclear calcium signal, targets cytoplasmic calcium only
#
...
#   2. Restart Fiji
#   3. Run via Plugins > CalciumAnalysis Wizard v1.1 Auto
...
CONFIG = {
    ...
    "ring_expansion_pixels": 5,  # v1.1: pixels to expand for cytoplasmic ring
...
def create_cytoplasmic_ring_roi(nucleus_roi, img_width, img_height, expansion_pixels=5):
    """
    Create a cytoplasmic ring ROI by dilating the nucleus and XORing with original.
    Returns a donut-shaped ShapeRoi (C = B - A where A is nucleus, B is dilated nucleus).
    """
    from ij.gui import PolygonRoi, Roi
    from ij.process import ByteProcessor
    
    # Create a small mask image to work with the nucleus
    # Get bounding rectangle with margin
    bounds = nucleus_roi.getBounds()
    margin = expansion_pixels + 5
    x_off = max(0, bounds.x - margin)
    y_off = max(0, bounds.y - margin)
    w = min(img_width - x_off, bounds.width + 2 * margin)
    h = min(img_height - y_off, bounds.height + 2 * margin)
    
    # Create mask for nucleus (A)
    mask_a = ByteProcessor(w, h)
    mask_a.setColor(0)
    mask_a.fill()
    
    # Translate ROI to local coordinates
    nucleus_local = nucleus_roi.clone()
    nucleus_local.setLocation(bounds.x - x_off, bounds.y - y_off)
    
    # Fill nucleus in mask A
    mask_a.setColor(255)
    mask_a.setRoi(nucleus_local)
    mask_a.fill(nucleus_local)
    
    # Create mask for dilated nucleus (B)
    mask_b_imp = ImagePlus("dilate_temp", mask_a.duplicate())
    for _ in range(expansion_pixels):
        IJ.run(mask_b_imp, "Dilate", "")
    mask_b = mask_b_imp.getProcessor()
    mask_b_imp.close()
    
    # Create ROI from mask B (dilated nucleus)
    mask_b_imp = ImagePlus("b_temp", mask_b)
    mask_b_imp.getProcessor().setThreshold(128, 255, 0)
    IJ.run(mask_b_imp, "Create Selection", "")
    roi_b_local = mask_b_imp.getRoi()
    mask_b_imp.close()
    
    if roi_b_local is None:
        IJ.log("Warning: Could not create dilated ROI")
        return None
    
    # Create ROI from mask A (nucleus)
    mask_a_imp = ImagePlus("a_temp", mask_a)
    mask_a_imp.getProcessor().setThreshold(128, 255, 0)
    IJ.run(mask_a_imp, "Create Selection", "")
    roi_a_local = mask_a_imp.getRoi()
    mask_a_imp.close()
    
    if roi_a_local is None:
        IJ.log("Warning: Could not create nucleus ROI")
        return None
    
    # XOR to get ring (C = B - A)
    shape_b = ShapeRoi(roi_b_local)
    shape_a = ShapeRoi(roi_a_local)
    ring_roi = shape_b.xor(shape_a)
    
    # Translate back to global coordinates
    ring_roi.setLocation(ring_roi.getBounds().x + x_off, ring_roi.getBounds().y + y_off)
    
    return ring_roi


def convert_nuclei_to_rings(nucleus_rois, img_width, img_height, expansion_pixels=5):
    """
    Convert a list of nucleus ROIs to cytoplasmic ring ROIs.
    """
    ring_rois = []
    for i, nucleus_roi in enumerate(nucleus_rois):
        ring_roi = create_cytoplasmic_ring_roi(
            nucleus_roi, img_width, img_height, expansion_pixels
        )
        if ring_roi is not None:
            ring_rois.append(ring_roi)
        else:
            IJ.log("Warning: Could not create ring for nucleus %d, using original" % (i+1))
            ring_rois.append(nucleus_roi)
    return ring_rois
...
def auto_segment_all_rois(merged_imp, ts_imp_systole_frame):
    ...
    cyan_nucleus_rois = select_top_rois(cyan_rois_with_area, n_cyan)
    magenta_nucleus_rois = select_top_rois(magenta_rois_with_area, n_magenta)
    
    # === V1.1: Convert nucleus ROIs to cytoplasmic ring ROIs ===
    img_width = merged_imp.getWidth()
    img_height = merged_imp.getHeight()
    expansion = CONFIG["ring_expansion_pixels"]
    
    IJ.log("Creating cytoplasmic ring ROIs (expansion=%d px)..." % expansion)
    cyan_rois = convert_nuclei_to_rings(cyan_nucleus_rois, img_width, img_height, expansion)
    magenta_rois = convert_nuclei_to_rings(magenta_nucleus_rois, img_width, img_height, expansion)
    IJ.log("  Created %d Cyan ring ROIs" % len(cyan_rois))
    IJ.log("  Created %d Magenta ring ROIs" % len(magenta_rois))
